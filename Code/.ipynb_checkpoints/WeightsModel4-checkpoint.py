...
class PreProcessing:
    ...
    def preprocess_global_data(self, X, y, products, timePeriods, timePeriodsTestStart, tau, features, features_to_scale,
        ...
        train_features = (timePeriods <= timePeriodsTestStart)

        # Fit feature scalers per product (on training horizon)
        X_scalers = {}
        for product in np.unique(products):
            scaler = copy.deepcopy(X_scaler, **kwargs)
            X_scalers[product] = scaler.fit(X[train_features & (products == product)], features, features_to_scale, features_to_scale_with)

        # Select training horizons for demand scaling
        train_demands = (timePeriods < timePeriodsTestStart)
        
        # Fit demand scalers per product (on training horizon)
        y_scalers = {}
        for product in np.unique(products):
            scaler = copy.deepcopy(y_Scaler)
            y_scalers[product] = scaler.fit(y[train_demands & (products == product)], tau=tau, **kwargs)

        # Reshape demand time series to multi-period 
        data = pd.DataFrame({'products': products, 'timePeriods': timePeriods, 'y': y.flatten()})
        y_mp = {}
        for tau_ in range(tau+1):
            y_mp['y'+str(tau_)] = data.groupby(['products']).shift(-tau_)['y']
        y = np.array(pd.DataFrame(y_mp))

        # Select train / test horizons considering multi-period demands given tau
        train, test = timePeriods < timePeriodsTestStart - tau, timePeriods == timePeriodsTestStart

        # Select train / test data
        X_train, X_test = X[train], X[test]
        y_train, y_test = y[train], y[test]
        products_train, products_test = products[train], products[test]
        timePeriods_train, timePeriods_test = timePeriods[train], timePeriods[test]

        # Rehsape training data aligned to periods of the rolling look ahead horizon
        X_train = self.reshape_data(X_train, timePeriods_train, tau)
        y_train = self.reshape_data(y_train, timePeriods_train, tau)
        products_train = self.reshape_data(products_train, timePeriods_train, tau)
        timePeriods_train = self.reshape_data(timePeriods_train, timePeriods_train, tau)

        # Flatten 1-d arrays
        products_train, products_test = products_train.flatten(), products_test.flatten()
        timePeriods_train, timePeriods_test = timePeriods_train.flatten(), timePeriods_test.flatten()
        if tau == 0:
        ...
        data = (
            ...
            timePeriods_train, timePeriods_test, 
            X_scalers, y_scalers
        )
